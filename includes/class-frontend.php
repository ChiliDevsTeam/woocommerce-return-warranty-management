...
class WCRW_Frontend {
    ...
    public function show_warranty() {
        ...
        if ( $warranty['type'] == 'included_warranty' ) {
            if ( $warranty['length'] == 'limited' ) {
                ...
                echo '<p class="wcrw_warranty_info"><strong>'. $warranty_label .':</strong> '. __( 'Lifetime', 'wc-return-warranty-management' ) .'</p>';
            }
            ...
            if ( is_array($addons) && !empty($addons) ) {
                ...
                foreach ( $addons as $x => $addon ) {
                    ...
                    if ( $value == 0 && $price == 0 ) {
                        ...
                        if ( $price == 0 ) {
                            $price = __( 'Free', 'wc-return-warranty-management' );
                        } else {
    ...
    public function add_cart_validation( $valid = '', $product_id = 0 ) {
        ...
        if ( $warranty['type'] == 'addon_warranty' && ! isset( $_REQUEST['wcrw_warranty'] ) ) {
            $error = sprintf( __( 'Please select your %s first.', 'wc-return-warranty-management' ), $warranty_label );
        ...
        if ( $warranty ) {
            if ( $warranty['type'] == 'addon_warranty' && isset( $cart_item['wcrw_warranty_index'] ) ) {
                ...
                if ( $warranty['length'] == 'lifetime' ) {
                    $other_data[] = array(
                        'name'      => $warranty_label,
                        'value'     => __( 'Lifetime', 'wc-return-warranty-management' ),
                        'display'   => ''
                    );
